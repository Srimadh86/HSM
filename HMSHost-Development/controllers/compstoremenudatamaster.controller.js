...
const StoreMenuData = db.menustoredata;
exports.list = async(req,res)=>{
    const menustoredatas = await StoreMenuData.findAll({order: [['CSM_ID', 'DESC']]});
    if(menustoredatas)
    {
        res.json({"status":"Success","message":"menustoredatas List",'data':menustoredatas});
    }
    else
    {
...
exports.add = async(req,res)=>{
    var menustoredata = await StoreMenuData.findOne({where:{Comp_Store_Menu_Item_Name:req.body.Comp_Store_Menu_Item_Name}})
    req.body.status = 'Active';
    req.body.ipAddress = requestIp.getClientIp(req);
    if(!menustoredata)
    {
    var addstatus = StoreMenuData.create(req.body);
    if(addstatus)
    {
